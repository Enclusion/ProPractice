...
public class PartyMenu {
	...
	public static void openMenu(Player player) {
		IconMenu menu = new IconMenu(MessageUtils.color(PracticeConfiguration.getRootConfig().getConfig().getString("menus.party-matchmaking")), Practice.getInstance().getGameManager().getGameMenuAmount(), event -> {
	        ...
	        if(!data.inParty()) {
	        	player.sendMessage(MessageUtils.getTranslation("errors.menu-party"));
	        	return;
	        }
	        
	        if(event.getName() != null) {
	            player.closeInventory();
	            Practice.getInstance().getGameManager().addToMatchMaker(event.getUUID(), data.getParty());
	        }
	    }, null);
